package com.yuanlin.o2o.controller.local;
...
public class LocalAuthController {
	...
	private Map<String, Object> changeLocalPwd(HttpServletRequest request) {
		...
		if (userName != null && password != null && newPassword != null && user != null && user.getUserId() != null
			...
			try {
				...
				LocalAccount localAuth = localAuthService.getLocalAuthByUserId(user.getUserId());
				if (localAuth == null || !localAuth.getLocalAccountName().equals(userName)) {
					// 不一致则直接退出
					modelMap.put("success", false);
	...
	private Map<String, Object> logincheck(HttpServletRequest request) {
		...
		if (userName != null && password != null) {
			// 传入帐号和密码去获取平台帐号信息
			LocalAccount localAuth = localAuthService.getLocalAuthByUsernameAndPwd(userName, password);
			if (localAuth != null) {
				// 若能取到帐号信息则登录成功
				modelMap.put("success", true);
				// 同时在session里设置用户信息
