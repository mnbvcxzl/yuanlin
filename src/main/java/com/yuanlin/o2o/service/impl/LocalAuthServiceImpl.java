...
import com.yuanlin.o2o.dao.LocalAuthDao;
...
import com.yuanlin.o2o.service.LocalAuthService;
import com.yuanlin.o2o.util.MD5;
@Service
public class LocalAuthServiceImpl implements LocalAuthService {
	...
	@Override
	public LocalAccount getLocalAuthByUsernameAndPwd(String username, String password) {
		return localAuthDao.queryLocalByUserNameAndPwd(username, MD5.getMd5(password));
	}

	@Override
	public LocalAccount getLocalAuthByUserId(long userId) {
		return localAuthDao.queryLocalByUserId(userId);
	}

	@Override
	@Transactional
	public LocalAuthExecution bindLocalAuth(LocalAccount localAuth) throws LocalAuthOperationException {
		// 空值判断，传入的localAuth 帐号密码，用户信息特别是userId不能为空，否则直接返回错误
		if (localAuth == null || localAuth.getPassword() == null || localAuth.getLocalAccountName() == null
				|| localAuth.getUserInfo() == null || localAuth.getUserInfo().getUserId() == null) {
			return new LocalAuthExecution(LocalAuthStateEnum.NULL_AUTH_INFO);
		}
		// 查询此用户是否已绑定过平台帐号
		LocalAccount tempAuth = localAuthDao.queryLocalByUserId(localAuth.getUserInfo().getUserId());
		if (tempAuth != null) {
